    ...
    updateById: function(req, res, next) {
        movieModel.findByIdAndUpdate(req.params.movieId,{name:req.body.name}, {new: true},function(err, movieInfo){
            if(err)
                ...
                res.json({status:"success", message: "Movie updated successfully!!!", data: {name: movieInfo.name, released_on: movieInfo.released_on}});
            }
        });
    },
    ...
    create: function(req, res, next) {
        movieModel.create({ name: req.body.name, released_on: req.body.released_on }, function (err, result) {
            if (err)
                ...
                res.json({status: "success", message: "Movie added successfully!!!", data: {
                    name: result.name,
